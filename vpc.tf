...
# Secondary CIDR Subnets private
resource "aws_subnet" "demo-secondary-private-1" {
  vpc_id     = aws_vpc.demo.id
  cidr_block = var.secondary_subnet[0]
  availability_zone = data.aws_availability_zones.available.names[0]
  map_public_ip_on_launch = "false"
  depends_on    = [aws_vpc_ipv4_cidr_block_association.secondary_cidr]

  tags = {
    Name = "secondary-demo-private-1"
  }
}

resource "aws_subnet" "demo-secondary-private-2" {
  vpc_id     = aws_vpc.demo.id
  cidr_block = var.secondary_subnet[1]
  availability_zone = data.aws_availability_zones.available.names[1]
  map_public_ip_on_launch = "false"
  depends_on    = [aws_vpc_ipv4_cidr_block_association.secondary_cidr]

  tags = {
    Name = "secondary-demo-private-2"
  }
}
